...
Hard_nodes = inpolygon(NN_Copy(:,3),NN_Copy(:,2),Poly_new(:,2),Poly_new(:,1));
...
no_of_simul = 1000;
 ...
 for p = 1:1:length(no_of_towers)
     ...
     for q = 1:1:length(Hard_ratio)
        ...
        LOF_CB_FII_1(p,q) = (1- mean(LOF_array))*100;
        Damage_CB_FII_1(p,q) = mean(Damage_Matrix(:,3));
        fprintf('towers%d\n',nt)
        fprintf('hard%d\n',hr)
...
save('LOF_CB_FII_1','LOF_CB_FII_1')
save('Damage_CB_FII_1','Damage_CB_FII_1')
