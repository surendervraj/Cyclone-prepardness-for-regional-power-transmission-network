...
a = polyarea(Strip2(:,3),Strip2(:,2));

% Identifying the towers inside the polygon
Hard_nodes = inpolygon(NN_Copy(:,3),NN_Copy(:,2),Strip2(:,3),Strip2(:,2));
Rows = find(Hard_nodes == 1);
...
for i = 1:1:length(Edges_inside)
    ...
    Edges_inside(i,2) = Edges(Edge11,5);
end
...
no_of_simul = 1000;
 ...
 for p = 1:1:length(no_of_towers)
     ...
     for q = 1:1:length(Hard_ratio)
        ...
        LOF_IS875_Pop_1(p,q) = (1- mean(LOF_array))*100;
        Damage_IS875_Pop_1(p,q) = mean(Damage_Matrix(:,3));
        fprintf('towers%d\n',nt)
        fprintf('hard%d\n',hr)
...
save('LOF_IS875_Pop_1','LOF_IS875_Pop_1')
save('Damage_IS875_Pop_1','Damage_IS875_Pop_1')
